...
const MessageInput = ({ onSendData, isLoading, errorText,clearError }) => {
	const [text, setText] = useState("");
	let [istyping, setIsTyping] = useState(false);
  const [translateClass, setTranslateClass] = useState('translate-y-48');
  ...
  useEffect(() => {
    if (errorText ) {
      ...
      const timer = setTimeout(() => {
        setTranslateClass('translate-y-48');
        clearError(); 
      }, 3000);
  ...
  console.log(errorText); 
	return (
		<div className="flex items-center flex-col justify-between px-1 md:px-4 pt-4 w-full">
			...
			<div className={`flex items-center justify-center bg-red-500/40 fixed top-100 rounded-xl py-4 px-10 text-sm font-light duration-500  ${translateClass}`} id="errorDummy">
        {errorText}
